  ...
  <v-layout row wrap v-if="stock == undefined ? false : true">
    ...
    <v-flex md6 xs12>
      <v-layout row wrap>
        ...
        <v-flex class="borderBottle" xs6>
          <h1 class="text-xs-right green--text">
            <v-icon large class="success--text" pr-3>arrow_drop_up</v-icon>
            {{ stock.Finance.FinanceStatDaily.Lastprice }}
          </h1>
          <h4 class="text-xs-right grey--text">
            <v-icon small class="grey--text">access_time</v-icon>
            {{ stock.Finance.FinanceStatDaily.Date }}
          </h4>
        </v-flex>
        <v-flex xs12>
    ...
    data: function(val) {
      ...
      if (this.stock != undefined)
        this.items.forEach((item, i) => {
          switch (i) {
            case 0:
              item.valueMax = parseFloat(val.GrowthStock[0].Net_rate);
              break;
            case 1:
              item.valueMax = parseFloat(val.GrowthStock[0].Assets_rate);
              break;
            case 2:
              item.valueMax = parseFloat(val.GrowthStock[0].Price_rate);
              break;
            case 3:
              item.valueMax = parseFloat(val.StockDividend[0].DIv_rate);
              break;
            case 4:
              item.valueMax = parseFloat(val.StockDividend[0].More_one_rate);
              break;
            default:
              item.valueMax = parseFloat(val.StockDividend[0].Double_rate);
          }
          item.process = false;
          item.value = 0;
        });
